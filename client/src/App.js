...
class App extends Component {
  ...
  componentDidMount = async () => {
    document.title = "Distributed Health Care";
  ...
  async onSubmit(event) {
    ...
    this.report
      .sendReclamation(
        this.state.drugName,
        this.state.drugStoreAddress,
        this.state.ipfsHash,
        { from: this.state.account }
      )
      .then(() => {
        // Get the value from the contract to prove it worked.
        this.report.contract.getPastEvents(
          "reclamationReceived",
          {
            fromBlock: 0,
          },
  ...
  async search() {
    const response = await axios.get(`${this.state.baseUrl}/search`, {
      params: {
        term: this.state.searchTerm.toUpperCase(),
  ...
  render() {
    return (
      <div className="app-container" style={{ backgroundColor: "#001529" }}>
        ...
        <div className="mui-panel" style={{ backgroundColor: "#001529" }}>
          <div className="mui--text-headline" style={{ color: "white" }}>
            {this.state.numHits} Hits
          </div>
          <div className="mui--text-subhead" style={{ color: "white" }}>
            Displaying Results {this.state.searchOffset} -{" "}
            {this.state.searchOffset + 9}
          </div>
        </div>
        ...
        <div className="search-results">
          ...
          this.state.didAtLeastOneSearch ? (
            <div
              className="mui-panel"
              style={{
                display: "flex",
            ...
            >
              <span className="material-icons md-48" style={{ color: "white" }}>
                help
              </span>
              <div className="mui--text-title" style={{ color: "white" }}>
                {" "}
          ...
          {this.state.searchResults.map((hit, index) => {
            return (
              <div
                ...
                onClick={() => this.showMedicineModal(hit)}
                key={index}
              >
                <div className="mui--text-title" style={{ color: "white" }}>
                  {" "}
                  {hit._source.specialite}{" "}
                </div>
                <div
                  className="mui-divider"
                  style={{ textUnderline: "white" }}
                ></div>
                <div className="mui--text-subhead" style={{ color: "white" }}>
                  {" "}
                  {hit._source.dci}
                ...
                <div className="mui--text-body2" style={{ color: "white" }}>
                  {" "}
                  Dosage : {hit._source.dosage}
                </div>
                <div className="mui--text-body2" style={{ color: "white" }}>
                  {" "}
                  Forme : {hit._source.forme}
                </div>
              </div>
            );
          })}
        ...
        <div
          className="mui-panel pagination-panel"
          style={{
            position: "fixed",
            left: "40%",
            bottom: 0,
          ...
          >
            <div className="medicine-container">
              ...
              <div className="title-row" style={{ color: "white" }}>
                <div
                  className="mui--text-display2 all-caps"
                  style={{ color: "white" }}
                >
                  {this.state.selectedMedicine._source.specialite}
                </div>
                <div className="mui--text-display1" style={{ color: "white" }}>
          ...
          >
            <div className="medicine-container">
              ...
              <form onSubmit={this.onSubmit}>
                ...
                <div className="mui-textfield">
                  <input
                    ...
                    style={{ color: "white" }}
                    onChange={(event) =>
                      this.setState({ drugStoreAddress: event.target.value })
                    }
                  />
              ...
              >
                <button
                  className="mui-btn mui-btn--flat"
                  onClick={() => this.setState({ showRecModal: false })}
                  style={{ color: "white" }}
                >
                  Close
                </button>
